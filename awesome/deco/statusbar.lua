...
-- local task_tag_list      = require("deco.tag-task-list.taglist")
-- local control_center     = require("deco.widgets.control-center")
...
mytextclock              = wibox.widget.textclock(
  '<span color="#ffffff" font="Inter Medium Font 12"> %a %b %d, %H:%M </span>',
  10)
...
awful.widget.watch(battery_script, 5, function(widget, stdout)
  ...
  if battery_level then
    battery_text:set_text("Battery: " .. tostring(battery_level) .. "%")
  else
    battery_text:set_text("Battery: N/A")
  end
end)
  ...
  {
    {
      {
        widget = wibox.widget.imagebox,
        image = os.getenv("HOME") .. "/.icons/papirus-icon-theme-20230301/Papirus/64x64/devices/battery.svg",
        resize = true,
        -- forced_height = 20,
        -- forced_width = 20,
      },
...
battery_text.font = "CaskaydiaCove Nerd Font 13"
      ...
      {
        {
          id = "screenshot",
          widget = wibox.widget.imagebox,
          image = os.getenv("HOME") .. "/.icons/papirus-icon-theme-20230301/Papirus/64x64/devices/audio-headset.svg",
          resize = true,
          opacity = 0,
        },
        forced_height = 20,
...
brightness_slider:connect_signal("property::value", function(slider)
  ...
  awful.spawn.easy_async("light -S " .. brightness_level, function()
    -- slider:get_children_by_id("brightness_value")[1].text = "Brightness: " .. tostring(slider.value) .. "%"
  end)
end)
...
volume_slider:connect_signal("property::value", function(slider)
  ...
  awful.spawn.easy_async("amixer set Master" .. volume_level, function()
    -- slider:get_children_by_id("volume_value")[1].text = "volume: " .. tostring(slider.value) .. "%"
  end)
end)
...
awful.screen.connect_for_each_screen(function(s)
  ...
  ff_launch:buttons(gears.table.join(awful.button({}, 1, function() awful.spawn("firefox", false) end)))
  ...
  -- the wibox
  s.mywibox = awful.wibar({ position = "top", screen = s, height = 30, opacity = 0.7, fg = '#ffffff', bg = '#00000099' })
    ...
    expand = "none",
    {
      ...
      halign = "center",
      {
        ...
        },
        {
          id = "vokoscreenNG",
          widget = wibox.widget.imagebox,
          image = os.getenv("HOME") .. "/.icons/papirus-icon-theme-20230301/Papirus/32x32/apps/vokoscreenNG.svg",
          resize = false,
        },
        {
          id = "kdeconnect",
          widget = wibox.widget.imagebox,
          image = os.getenv("HOME") .. "/.icons/papirus-icon-theme-20230301/Papirus/32x32/apps/kdeconnect.svg",
          resize = false,
        },
        {
          id = "keepassxc",
